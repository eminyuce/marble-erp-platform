...
test.describe('Block Management (Ocak & Bloklar)', () => {
  ...
  test('Blocks list page loads with Tabulator grid and filters', async ({ page }) => {
    ...
    const pageTotals = await page.evaluate(() => {
      const tableEl = document.getElementById('blocks-table');
      const table = tableEl && tableEl.tabulator;
      const rows = table && typeof table.getData === 'function'
        ? table.getData().filter((row) => row && row.id != null)
        : [];
      const num = (value) => {
      ...
      const expected = rows.reduce((sum, row) => {
        const actual = num(row.actualTonnage);
        sum.tonnage += actual > 0 ? actual : num(row.approximateTonnage);
        sum.m2 += num(row.surfaceAreaM2);
        return sum;
      }, { count: rows.length, tonnage: 0, m2: 0 });
      const parseTr = (text) => Number(String(text || '')
        .replace(/[^\d,-]/g, '')
        .replace(/\./g, '')
        .replace(',', '.'));
      return {
        expected,
        displayedTonnage: parseTr(document.getElementById('blocks-total-tonnage')?.textContent),
        displayedM2: parseTr(document.getElementById('blocks-total-m2')?.textContent)
      };
    });
    expect(pageTotals.expected.count).toBeGreaterThan(0);
    expect(pageTotals.displayedTonnage).toBeCloseTo(pageTotals.expected.tonnage, 1);
    expect(pageTotals.displayedM2).toBeCloseTo(pageTotals.expected.m2, 1);

    const sizeSelect = page.locator('#blocks-table .tabulator-page-size');
    if (await sizeSelect.count()) {
      const sizeResponse = page.waitForResponse(res => res.url().includes('/blocks/api/data') && res.status() === 200);
      await sizeSelect.selectOption('5');
      await sizeResponse;
      await waitForTabulator(page, '#blocks-table', 1);
      const afterSizeChange = await page.evaluate(() => {
        const tableEl = document.getElementById('blocks-table');
        const table = tableEl && tableEl.tabulator;
        const rows = table && typeof table.getData === 'function'
          ? table.getData().filter((row) => row && row.id != null)
        ...
        const tonnage = rows.reduce((sum, row) => {
          const actual = num(row.actualTonnage);
          return sum + (actual > 0 ? actual : num(row.approximateTonnage));
        }, 0);
        const parseTr = (text) => Number(String(text || '')
          .replace(/[^\d,-]/g, '')
          .replace(/\./g, '')
      ...
      expect(afterSizeChange.count).toBeLessThanOrEqual(5);
      expect(afterSizeChange.displayedTonnage).toBeCloseTo(afterSizeChange.expectedTonnage, 1);
    }
