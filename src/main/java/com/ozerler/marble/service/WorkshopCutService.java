...
public class WorkshopCutService {
    ...
    public CutOrder createCutOrder(Long projectId, Long locationId, Long slabId,
        ...
        CutOrder order = CutOrder.builder()
                .cutOrderNo("CUT-" + Year.now().getValue() + "-" + System.currentTimeMillis() % 100000)
                .project(project)
                .location(location)
        ...
        BigDecimal itemArea = targetWidthCm.multiply(targetLengthCm)
                .divide(new BigDecimal("10000"), 4, RoundingMode.HALF_UP);

        // Unit cost loaded with workshop cutting expense (+15%)
        BigDecimal unitCost = sourceSlab.getCostPerM2().multiply(new BigDecimal("1.15")).setScale(2, RoundingMode.HALF_UP);

        List<CutItem> items = new ArrayList<>();
        long seq = System.currentTimeMillis() % 10000;
        BigDecimal totalCutArea = BigDecimal.ZERO;

        for (int i = 1; i <= piecesCount; i++) {
            CutItem item = CutItem.builder()
                    .itemCode("ITM-" + Year.now().getValue() + "-" + (seq++))
                    .cutOrder(order)
                    ...
                    .areaM2(itemArea)
                    .edgeFinish(edgeFinish != null ? edgeFinish : "PAHLI_CILALI")
                    .unitCost(unitCost)
                    .targetLocation(targetLocationDesc != null ? targetLocationDesc : (location != null ? location.getLocationName() : "Genel"))
                    .status("READY")
                    .build();
            items.add(item);
            totalCutArea = totalCutArea.add(itemArea);
        }

        cutItemRepository.saveAll(items);

        // Calculate Workshop Waste (Nesting / Edge scrap: FR-09)
        ...
        if (scrapArea.compareTo(BigDecimal.ZERO) > 0) {
            ScrapLog scrap = ScrapLog.builder()
                    .scrapCode("SCRAP-W-" + (seq++))
                    .cutOrder(order)
                    .slab(sourceSlab)
                    .block(sourceSlab.getBlock())
                    .reasonCode(ScrapReasonCode.FR_09)
                    .scrapAreaM2(scrapArea)
                    .costImpact(scrapArea.multiply(sourceSlab.getCostPerM2()).setScale(2, RoundingMode.HALF_UP))
